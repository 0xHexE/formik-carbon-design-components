...
function getOutputs({ file }) {
  ...
  const shouldUseIndex = () => {
    return (
      ...
      fileBaseName.replace(path.extname(fileBaseName), '') ===
        fileDirectory.replace(fileDirectoryParent, '').slice(1)
    );
  };

  return buildFormats.map((format) => {
    const fileDistDirectory = fileDirectory
      .replace('src', 'dist')
      .replace(distDirectory, '');
    return {
      file: shouldUseIndex()
...
export default getFilesFolders(srcDirectory)
  ...
  .map((file) => ({
    ...
    plugins: [
      ...
      postcss({
        modules:
          fs.existsSync(file.replace('.js', '.module.scss')) ||
          fs.existsSync(file.replace('.js', '.module.css')),
      }),
      terser(),
    ],
    external: ['prop-types'],
